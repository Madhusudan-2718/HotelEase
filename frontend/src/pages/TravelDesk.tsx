...
export default function TravelDesk({ roomNumber = "305", onBack }: TravelDeskProps) {
  ...
  useEffect(() => {
    const fetchBookingHistory = async () => {
      try {
        // TODO: Replace with actual API call when backend is ready
        // const bookings = await travelDeskApi.getBookings(roomNumber);
        // const formattedBookings: BookingHistoryItem[] = bookings.map((booking: TravelBooking) => ({
        //   id: booking.id,
        ...
        // For now, set empty array - will be populated from API
        setBookingHistory([]);
      } catch (err) {
  ...
  const handleBookingSubmit = async (bookingData: any) => {
    try {
      ...
      // TODO: Replace with actual API call when backend is ready
      // const createdBooking = await travelDeskApi.createBooking(bookingPayload);
      
      // For now, create a temporary booking object
      ...
      const newBooking: BookingHistoryItem = {
        ...
        status: "new",
        date: new Date(bookingData.date).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
        time: bookingData.time,
